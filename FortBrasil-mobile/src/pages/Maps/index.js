...
export default function Maps({navigation}) {
  ...
  const listEstablishments = useSelector(state => ({
    list: state.establishment || [],
  }));
  useEffect(() => {
    async function loadData() {
      ...
      Toast.loading(false);
      Toast.info(
        `${listEstablishments.list.length} Estabelecimento(s) cadastrado(s)`,
      );
    }
    loadData();
  }, []);
  ...
  function handleSetAllEstablishment(establishments) {
    dispatch(EstablishmentActions.setEstablishment(establishments));
  }

  function handleAddEstablishment(establishment) {
    Toast.loading(true);
  ...
  function update(establishment, updateEstablishment) {
    dispatch(
      EstablishmentActions.updateStablishmentRequest(
        establishment,
        updateEstablishment,
      ),
    );
  ...
  async function getList() {
    ...
    handleSetAllEstablishment(response.data);
  }
  ...
  function toggleModal() {
    if (isFormModalVisible === true) {
      setEstablishmentUpdate(null);
    }
  ...
  const [apikey, setApikey] = useState(
    'AIzaSyC7rk__5hEwhpaGeAfPYNHJSDJ9lAPKW4c',
  );
  const [user, setUser] = useState({});
  const [isCreate, setIsCreate] = useState(false);
  const [isFormModalVisible, setIsFormModalVisible] = useState(false);
  const [isInfoModalVisible, setInfoFormModalVisible] = useState(false);
  const [establishmentUpdate, setEstablishmentUpdate] = useState(false);

  return (
    <>
      <MapView style={stylesSheet.stylesMaps} initialRegion={initialRegion}>
        {listEstablishments.list.map(establishment => {
          return (
            <Marker
              ...
              <Callout
                onPress={() => {
                  setEstablishmentUpdate(establishment);
                  setIsCreate(false);
